...
両方を同時に使うことはできません。もし英語がうまくなりたければ、一日の時間を英語に割り当てる必要があります。
...
2. 思考言語の切り替えが言語学習において最も重要であり、それにはスピーキング（思考）が必要だからです。
...
発音は、あなたがキャスターのようなプロの話し手になりたいのでない限り、無意味です。
...
1. 安いポータブルオーディオプレーヤーを買ってください。充電切れ（ダウンタイム）を防ぐために2台持っておきたいところです。
    * これは重要です。スマホは動作が遅く、大きく、高価だからです。シンプルな道具が必要です。
...
3. 動画から音声を抽出する。
4. 字幕ファイルのタイムスタンプを使って、Python等で音声の一部（セリフ）を切り出す。
